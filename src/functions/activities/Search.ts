...
export class Search extends Workers {
    ...
    public async doSearch(page: Page, data: DashboardData) {
        ...
        // Human-like delay after loading Bing (2-4 seconds)
        ...
        if (missingPoints > 0) {
            ...
            // Human-like delay before generating extra searches (1-2 seconds)
    ...
    private async bingSearch(searchPage: Page, query: string) {
        ...
        for (let i = 0; i < 5; i++) {
            try {
                ...
                // Human-like delay after scrolling (0.3-1 second)
                ...
                // Human-like delay after focusing (0.3-1 second)
                await this.bot.utils.wait(this.bot.utils.randomNumber(300, 1000))
                ...
                if (this.bot.config.searchSettings.scrollRandomResults) {
                    // Human-like delay before scrolling (1-3 seconds)
                    await this.bot.utils.wait(this.bot.utils.randomNumber(1000, 3000))
                    await this.randomScroll(resultPage)
                }

                if (this.bot.config.searchSettings.clickRandomResults) {
                    // Human-like delay before clicking (1-3 seconds)
                    await this.bot.utils.wait(this.bot.utils.randomNumber(1000, 3000))
                    await this.clickRandomLink(resultPage)
                }

                // Delay between searches (using existing config-based delay)
                ...
                // Additional human-like delay after search completion (1-3 seconds)
                await this.bot.utils.wait(this.bot.utils.randomNumber(1000, 3000))
    ...
    private async getGoogleTrends(geoLocale: string = 'US'): Promise<GoogleSearch[]> {
        ...
        // Human-like delay before fetching trends (1-2 seconds)
        await this.bot.utils.wait(this.bot.utils.randomNumber(1000, 2000))
    ...
    private async getRelatedTerms(term: string): Promise<string[]> {
        // Human-like delay before fetching related terms (0.5-1.5 seconds)
        await this.bot.utils.wait(this.bot.utils.randomNumber(500, 1500))
    ...
    private async clickRandomLink(page: Page) {
        try {
            ...
            // Stay for 10-15 seconds for page to load and "visit"
            await this.bot.utils.wait(this.bot.utils.randomNumber(10000, 15000))
